...
def main():
    ...
    style.map(
        ...
        foreground=[("selected", "black")]      # texto pestaña activa → negro
    )
    ...
    entrada = tk.Entry(terminal_frame, width=80, **ENTRY_STYLE)
    entrada.pack(pady=5)
    ...
    # Explorador / Editor
    # ---------------------------
    explorer_frame = tk.Frame(notebook, bg="#564F81")  # morado
    notebook.add(explorer_frame, text="Explorador")

    label = tk.Label(
        explorer_frame,
        text="Abrir y editar archivos desde aquí",
        bg="#4E477B",
        fg="white",
        font=("Arial", 12, "bold")
    )
    ...
    label_proc = tk.Label(
        process_frame,
        text="Ver y administrar procesos del sistema",
        bg="#f5f5dc",
        fg="black",
        font=("Arial", 12, "bold")
    )
